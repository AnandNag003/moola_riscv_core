...
module moola_regfile (
  ...
  // Write Port from Write-Back Stage (Synchronous)
  ...
  always_comb begin
    if (rs1_addr == 5'd0) begin
      rs1_data = 32'h0;
    end else if (reg_write && (rd_addr == rs1_addr)) begin
      rs1_data = rd_data; // Internal bypass when writing and reading same register
    end else begin
  ...
  always_comb begin
    if (rs2_addr == 5'd0) begin
      rs2_data = 32'h0;
    end else if (reg_write && (rd_addr == rs2_addr)) begin
      rs2_data = rd_data; // Internal bypass when writing and reading same register
    end else begin
